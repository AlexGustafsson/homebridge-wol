...
class Computer {
  constructor(log, config) {
    ...
    this.log = this.config.logging ? log : () => {};
  ...
  setOnline(newState, callback) {
    ...
    if (newState !== currentState) {
      ...
      if (newState && this.config.mac) {
        this.log('Computer awake cycle started for "%s" (%s)', this.config.name, this.config.ip || 'unkown ip');
      ...
      if (!newState && !this.shuttingDown && this.config.shutdownCommand) {
        this.log('Computer shutdown cycle started for "%s" (%s)', this.config.name, this.config.ip || 'unkown ip');
