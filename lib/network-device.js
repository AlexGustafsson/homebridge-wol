...
class NetworkDevice {
  constructor(log, config) {
    this.config = {
      ...
      model: 'NetworkDevice',
      serialNumber: new Array(4).fill(null).map(_ => Math.round(Math.random() * 1e5).toString()).join('-'),
  ...
  setStatus(newStatus) {
    ...
    if (newStatus !== this.status) {
      ...
      this.switchService.getCharacteristic(Characteristic.On).updateValue(this.status === Status.WakingUp || this.status === Status.Online);
    }
  }
