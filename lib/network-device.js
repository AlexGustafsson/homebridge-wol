...
class NetworkDevice {
  ...
  setOnline(newState, callback) {
    ...
    if (shouldBeOnline && this.config.mac) {
    ...
    if (!shouldBeOnline && this.config.shutdownCommand) {
