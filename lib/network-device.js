...
class NetworkDevice {
  constructor(log, config) {
    this.config = Object.assign({
      ...
      esxiVMid: 0
      esxiHostIp: null,
      esxiHostSSHUsername: null,
      esixHostSSHPassword: null
    }, config);
  ...
  boot_esxi_virtual_machine() {
    ...
    // Override .format in string to allow us to format the esxi boot command
    ...
    var defaultWakeCmdSprintF = "sshpass {0} ssh {1}@{2} 'vim-cmd vmsvc/power.on' {3}"

    const runnableWakeCmd = defaultWakeCmdSprintF.format(
      this.config.esxiHostSSHPassword, this.config.esxiHostSSHUsername,
       this.config.esxiHostIp, this.config.esxiVMid)

    exec(runnableWakeCmd, error => {
      if (error) {
