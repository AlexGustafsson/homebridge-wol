...
class NetworkDevice {
  ...
  setOnline(newState, callback) {
    ...
    if (newState !== currentState) {
      ...
      if (this.config.isEsxiMachine && this.config.esxiWakeCommand) {
          this.log('NetworkDevice boot cycle started for ESXi machine "%s" (%s) on host %s', this.config.name, this.config.ip || 'unknown ip', this.config.esxiHostIp)
      }
  ...
  boot_esxi_virtual_machine() {
    ...
    this.setStatus(Status.WakingUp);
    this.log(this.config.esxiWakeCommand);
