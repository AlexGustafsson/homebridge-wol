...
import type { NetworkDeviceConfig } from "./config"
...
export default class Pinger extends EventEmitter {
  ...
  async executePingCommand(): Promise<boolean> {
    ...
    try {
      await exec(this.config.pingCommand, {
        timeout: this.config.pingCommandTimeout,
      })
  ...
  async ping(): Promise<boolean> {
    ...
    const response = await ping.promise.probe(this.config.host, {
      timeout: this.config.pingTimeout / 1000,
    })
    this.log.debug(
      "Result of pinging: %s",
      response.alive ? "online" : "offline"
    )
  ...
  async pollState(immediate: boolean = false): Promise<void> {
    ...
    const positives = this.history.reduce(
      (positives, isPositive) => (isPositive ? positives + 1 : positives),
      0
    )
    this.log.debug("Got %d positives out of %d", positives, this.history.length)
    // Only change state if all measurements are the same
    if (positives === this.history.length) this.emit("stateChanged", true)
    else if (positives === 0) this.emit("stateChanged", false)
  ...
  async start(): Promise<void> {
    this.stop()
    this.log.debug(
      "Starting pinger at an interval of %d milliseconds",
